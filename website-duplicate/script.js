...
const runnerRightsCards = [
  {
    id: "education",
    name: "Right to Education",
    icon: "&#128218;",
    meaning: "Every child has the right to learn and go to school.",
    example: "If a child misses school, adults should help them return to learning."
  },
  {
    id: "protection",
    name: "Right to Protection",
    icon: "&#128737;",
    meaning: "Every child must be safe from harm, abuse, and unsafe work.",
    example: "If someone feels unsafe, they can tell a trusted adult or call 1098."
  },
  {
    id: "health",
    name: "Right to Health",
    icon: "&#127973;",
    meaning: "Every child deserves food, care, and medical help.",
    example: "A sick child should be taken to a doctor or health worker."
  },
  {
    id: "equality",
    name: "Right to Equality",
    icon: "&#129309;",
    meaning: "Every child deserves respect, no matter their background.",
    example: "A school should include every child and treat them kindly."
  },
  {
    id: "privacy",
    name: "Right to Privacy",
    icon: "&#128274;",
    meaning: "A child's private details and identity should be protected.",
    example: "Do not share a child's private photo, case, or address."
  },
  {
    id: "safety",
    name: "Right to Safety",
    icon: "&#128680;",
    meaning: "Children should know safe places, trusted adults, and help numbers.",
    example: "If lost or scared, a child can ask police, teachers, or Childline 1098."
  }
];

const runnerShopItems = [
  { id: "outfit-sun", type: "outfit", title: "Sunshine Jacket", cost: 60, icon: "&#129509;" },
  { id: "outfit-hero", type: "outfit", title: "Rights Hero Cape", cost: 120, icon: "&#129464;" },
  { id: "bag-space", type: "bag", title: "Galaxy School Bag", cost: 80, icon: "&#127890;" },
  { id: "bag-books", type: "bag", title: "Book Buddy Bag", cost: 110, icon: "&#128188;" },
  { id: "board-blue", type: "board", title: "Sky Skateboard", cost: 100, icon: "&#128761;" },
  { id: "board-star", type: "board", title: "Star Trail Board", cost: 160, icon: "&#127775;" },
  { id: "cap-comet", type: "cap", title: "Comet Cap", cost: 70, icon: "&#129506;" },
  { id: "cap-crown", type: "cap", title: "Champion Crown", cost: 180, icon: "&#128081;" },
  { id: "shoes-speed", type: "shoes", title: "Speed Shoes", cost: 90, icon: "&#128095;" },
  { id: "badge-kind", type: "badge", title: "Kind Helper Badge", cost: 75, icon: "&#127894;" },
  { id: "badge-safety", type: "badge", title: "Safety Star Badge", cost: 130, icon: "&#11088;" }
];

const runnerQuizQuestions = [
  {
    question: "What does the Right to Education mean?",
    options: ["Children can learn and go to school", "Children should work all day", "Only adults can study", "School is only for some children"],
    answer: 0,
    explain: "Correct. Every child has the right to learn."
  },
  {
    question: "If a child feels unsafe, what is a good first step?",
    options: ["Keep it secret forever", "Tell a trusted adult", "Share private photos", "Go alone with a stranger"],
    answer: 1,
    explain: "A trusted adult can help keep the child safe."
  },
  {
    question: "Why is privacy important for children?",
    options: ["So private details stay safe", "So everyone knows their address", "So photos can be shared anywhere", "So children cannot speak"],
    answer: 0,
    explain: "A child's identity and private details should be protected."
  },
  {
    question: "What does equality mean?",
    options: ["Every child gets respect", "Only rich children matter", "Only one child can play", "Children should be ignored"],
    answer: 0,
    explain: "Equality means children are treated with dignity and fairness."
  },
  {
    question: "Which number can children in India call for help?",
    options: ["1098", "0000", "12345", "99999"],
    answer: 0,
    explain: "Childline 1098 helps children in difficult or unsafe situations."
  }
];
...
let runnerQuizState = null;
let runnerPopup = null;
...
function normalizeUser(user) {
  ...
  user.runnerInventory = user.runnerInventory || [];
  user.runnerLoadout = user.runnerLoadout || {};
  user.runnerLoadout.outfit = user.runnerLoadout.outfit || "Starter Tee";
  user.runnerLoadout.bag = user.runnerLoadout.bag || "School Bag";
  user.runnerLoadout.board = user.runnerLoadout.board || "Classic Board";
  user.runnerLoadout.cap = user.runnerLoadout.cap || "Bright Cap";
  user.runnerLoadout.shoes = user.runnerLoadout.shoes || "Comfy Shoes";
  user.runnerLoadout.badge = user.runnerLoadout.badge || "Rights Rookie";
  user.gameStats = user.gameStats || {};
  user.gameStats.coinsEarned = Number(user.gameStats.coinsEarned || 0);
  user.gameStats.rightsCards = user.gameStats.rightsCards || {};
  user.gameStats.levelsCompleted = Number(user.gameStats.levelsCompleted || 0);
  user.gameStats.badges = user.gameStats.badges || [];
  user.gameStats.bestScore = Number(user.gameStats.bestScore || 0);
  user.gameStats.quizScore = Number(user.gameStats.quizScore || 0);
      ...
      ${
        user
          ? `<nav class="nav" aria-label="${t("aria.mainNav")}">
              ...
              <button type="button" data-jump="runner-game"><span class="nav-icon">&#127939;</span><span>Game</span></button>
            </nav>`
          : ""
      }
      ...
      <section class="dashboard-grid">
        <article class="panel hero-panel">
          ...
          <div>
            ...
            <p class="runner-loadout-line">&#127775; Runner style: ${user.runnerLoadout.outfit}, ${user.runnerLoadout.bag}, ${user.runnerLoadout.badge}</p>
          </div>
        </article>
        <article class="panel">
...
function renderRunnerGame(user) {
  const stats = user.gameStats || {};
  const totalCards = Object.values(stats.rightsCards || {}).reduce((sum, value) => sum + Number(value || 0), 0);
  const ownedCount = user.runnerInventory.length;
  const running = runnerState?.running;
  const missionText = runnerState
    ? "Mission: collect 3 rights cards and reach the safe zone."
    : "Mission: run through Rights City, collect rights cards, avoid unsafe obstacles.";
  return `
    <section class="runner-section" id="runner-game">
      <div class="runner-hero">
        <div>
          <p class="level-chip">&#127918; New Game Module</p>
          <h2>Rights City Runner</h2>
          <p class="simple-text">Run, collect coins, unlock avatar items, and learn child rights through quick missions.</p>
        </div>
        <button class="pill-button primary" type="button" data-action="runner-start">${running ? "Restart Mission" : "Start Runner Mission"}</button>
      </div>

      <div class="runner-stats">
        ${statCard("&#129689;", formatNumber(user.coins), "Profile coins")}
        ${statCard("&#127942;", formatNumber(stats.levelsCompleted || 0), "Runner levels")}
        ${statCard("&#128179;", formatNumber(totalCards), "Rights cards")}
        ${statCard("&#11088;", formatNumber(stats.bestScore || 0), "Best runner score")}
      </div>

      <div class="runner-layout">
        <article class="runner-play-card">
          <div class="runner-topline">
            <strong>${missionText}</strong>
            <span>${runnerState ? "Lives: " + formatNumber(runnerState.lives) + " | Coins: " + formatNumber(runnerState.coins) + " | Score: " + formatNumber(runnerState.score) : "Ready for a safe adventure"}</span>
          </div>
          <canvas id="runnerCanvas" class="runner-canvas" width="840" height="430" aria-label="Rights City Runner game area"></canvas>
          ${runnerPopup ? renderRunnerPopup(runnerPopup) : ""}
          <div class="runner-controls" aria-label="Runner controls">
            <button class="pill-button ghost" type="button" data-action="runner-left">&#11013; Left</button>
            <button class="pill-button ghost" type="button" data-action="runner-jump">&#11014; Jump</button>
            <button class="pill-button ghost" type="button" data-action="runner-slide">&#11015; Slide</button>
            <button class="pill-button ghost" type="button" data-action="runner-right">Right &#10145;</button>
          </div>
        </article>

        <aside class="runner-side">
          <article class="panel">
            <p class="level-chip">&#128640; Avatar Loadout</p>
            <h2>${user.name}'s runner</h2>
            <div class="runner-avatar-preview">
              <span class="avatar-big">${avatarIcon(user)}</span>
              <div>
                <p><strong>Outfit:</strong> ${user.runnerLoadout.outfit}</p>
                <p><strong>Bag:</strong> ${user.runnerLoadout.bag}</p>
                <p><strong>Board:</strong> ${user.runnerLoadout.board}</p>
                <p><strong>Badge:</strong> ${user.runnerLoadout.badge}</p>
              </div>
            </div>
          </article>
          <article class="panel">
            <p class="level-chip">&#128179; Rights Collected</p>
            <div class="rights-chip-grid">
              ${runnerRightsCards.map((card) => '<span class="rights-chip">' + card.icon + ' ' + card.name.replace("Right to ", "") + ': ' + formatNumber(stats.rightsCards?.[card.id] || 0) + '</span>').join("")}
            </div>
          </article>
        </aside>
      </div>
      ...
      <section class="runner-shop">
        <div class="screen-title mini-title">
          <div>
            <p class="level-chip">&#127775; Avatar Customization Shop</p>
            <h2>Use coins to unlock outfits, bags, boards, caps, shoes, and badges</h2>
          </div>
          <span class="level-chip">${formatNumber(ownedCount)} items unlocked</span>
        </div>
        <div class="runner-shop-grid">
          ${runnerShopItems.map((item) => renderRunnerShopCard(item, user)).join("")}
        </div>
      </section>
    </section>
  `;
...
function renderRunnerPopup(card) {
  return '<div class="runner-popup" aria-live="polite">' +
...
function renderRunnerQuiz() {
  if (!runnerQuizState) return "";
  if (runnerQuizState.complete) {
    return '<section class="runner-quiz-card">' +
      '<p class="level-chip">&#127881; Level Complete</p>' +
      '<h2>You scored ' + formatNumber(runnerQuizState.score) + ' out of ' + formatNumber(runnerQuizState.questions.length) + '</h2>' +
      '<p>' + runnerQuizState.feedback + '</p>' +
      '<button class="pill-button primary" type="button" data-action="runner-start">Play Next Mission</button>' +
    '</section>';
  }
  const question = runnerQuizState.questions[runnerQuizState.index];
  return '<section class="runner-quiz-card" id="runner-quiz">' +
    '<p class="level-chip">&#129504; Quick Rights Quiz</p>' +
    '<h2>' + question.question + '</h2>' +
    '<div class="answer-grid">' +
      question.options.map((option, index) => '<button class="answer-button" type="button" data-action="runner-quiz-answer" data-runner-answer="' + index + '">' + option + '</button>').join("") +
    '</div>' +
    '<p class="feedback">' + (runnerQuizState.feedback || "Choose the best answer to earn bonus coins.") + '</p>' +
  '</section>';
}

function renderRunnerShopCard(item, user) {
  const owned = user.runnerInventory.includes(item.id);
  const equipped = Object.values(user.runnerLoadout || {}).includes(item.title);
  const canBuy = user.coins >= item.cost && !owned;
  return '<article class="runner-shop-card ' + (owned ? 'owned' : 'locked') + '">' +
    '<div class="runner-shop-icon">' + item.icon + '</div>' +
    '<h3>' + item.title + '</h3>' +
    '<p>' + item.type.toUpperCase() + ' | ' + formatNumber(item.cost) + ' coins</p>' +
    (owned
      ? '<button class="pill-button ' + (equipped ? 'primary' : 'ghost') + '" type="button" data-action="runner-equip" data-runner-item="' + item.id + '">' + (equipped ? "Equipped" : "Equip") + '</button>'
      : '<button class="pill-button ' + (canBuy ? 'primary' : 'ghost') + '" type="button" data-action="runner-buy" data-runner-item="' + item.id + '" ' + (canBuy ? '' : 'disabled') + '>' + (canBuy ? "Buy item" : "Need coins") + '</button>') +
  '</article>';
}

function startRunnerGame() {
  stopRunnerLoop();
  runnerQuizState = null;
  runnerPopup = null;
  runnerState = {
    running: true,
    lane: 1,
  ...
  runnerTimer = window.setInterval(runnerGameTick, 90);
  trackEvent("runner_start");
  render();
  requestAnimationFrame(() => document.getElementById("runner-game")?.scrollIntoView({ behavior: "smooth", block: "start" }));
}
...
function runnerGameTick() {
  ...
  runnerState.tick += 1;
  runnerState.distance += 7;
  runnerState.score += 2;
  if (runnerState.jumpTicks > 0) runnerState.jumpTicks -= 1;
  if (runnerState.slideTicks > 0) runnerState.slideTicks -= 1;
  if (runnerState.tick % 10 === 0) spawnRunnerItem("coin");
  if (runnerState.tick % 23 === 0) spawnRunnerItem("card");
  if (runnerState.tick % 18 === 0) spawnRunnerItem("obstacle");
  runnerState.items.forEach((item) => {
    item.y += item.speed;
  });
  handleRunnerCollisions();
  runnerState.items = runnerState.items.filter((item) => item.y < 470 && !item.hit);
  if (runnerState.lives <= 0 || runnerState.distance >= 1250) completeRunnerLevel(runnerState.lives > 0);
  drawRunnerGame();
}

function spawnRunnerItem(type) {
  const lane = Math.floor(Math.random() * 3);
  const item = { type, lane, y: -30, speed: type === "obstacle" ? 20 : 16 };
  if (type === "card") item.card = runnerRightsCards[Math.floor(Math.random() * runnerRightsCards.length)];
  if (type === "obstacle") item.kind = Math.random() > 0.5 ? "barrier" : "low";
  runnerState.items.push(item);
}

function handleRunnerCollisions() {
  const userLane = runnerState.lane;
  const inAir = runnerState.jumpTicks > 0;
  const sliding = runnerState.slideTicks > 0;
  runnerState.items.forEach((item) => {
    if (item.hit || item.lane !== userLane || item.y < 315 || item.y > 390) return;
    if (item.type === "coin") {
      item.hit = true;
      runnerState.coins += 1;
      runnerState.score += 8;
    ...
    if (item.type === "card") {
      item.hit = true;
      runnerState.cardsCollected += 1;
      runnerState.collectedCardIds.push(item.card.id);
      if (item.card.id === "education") runnerState.educationCards += 1;
      runnerState.score += 35;
      runnerPopup = item.card;
      playCorrectAnswerSound();
      window.setTimeout(() => {
        runnerPopup = null;
        if (runnerState) render();
      }, 2600);
      render();
    }
    if (item.type === "obstacle") {
      const avoided = item.kind === "barrier" ? inAir : sliding;
      item.hit = true;
      if (avoided) {
        runnerState.score += 18;
        playCorrectAnswerSound();
      } else {
        runnerState.lives -= 1;
        runnerState.score = Math.max(0, runnerState.score - 12);
...
function completeRunnerLevel(success) {
  const user = getUser();
  if (!user || !runnerState) return;
  stopRunnerLoop();
  runnerState.running = false;
  const bonusCoins = success ? 20 : 8;
  const earnedCoins = runnerState.coins + bonusCoins + runnerState.cardsCollected * 5;
  user.coins += earnedCoins;
  user.points += success ? 20 : 8;
  user.gameStats.coinsEarned += earnedCoins;
  user.gameStats.levelsCompleted += success ? 1 : 0;
  user.gameStats.bestScore = Math.max(user.gameStats.bestScore || 0, runnerState.score);
  runnerState.collectedCardIds.forEach((cardId) => {
    user.gameStats.rightsCards[cardId] = Number(user.gameStats.rightsCards[cardId] || 0) + 1;
  });
  if (runnerState.cardsCollected >= 3) addRunnerBadge(user, "Card Collector");
  if (success) addRunnerBadge(user, "Safe Zone Finisher");
  if (runnerState.educationCards >= 3) addRunnerBadge(user, "Education Explorer");
  saveUser(user);
  trackEvent("runner_complete", { success, earnedCoins });
  runnerQuizState = makeRunnerQuizState();
  render();
  celebrateReward({ coins: earnedCoins, points: success ? 20 : 8 });
  requestAnimationFrame(() => document.getElementById("runner-quiz")?.scrollIntoView({ behavior: "smooth", block: "center" }));
}

function addRunnerBadge(user, badge) {
  if (!user.gameStats.badges.includes(badge)) user.gameStats.badges.push(badge);
  addBadge(user, badge);
}

function makeRunnerQuizState() {
  const shuffled = [...runnerQuizQuestions].sort(() => Math.random() - 0.5).slice(0, 3);
  return { index: 0, score: 0, questions: shuffled, feedback: "", complete: false };
}

function answerRunnerQuiz(index) {
  const user = getUser();
  if (!user || !runnerQuizState || runnerQuizState.complete) return;
  const question = runnerQuizState.questions[runnerQuizState.index];
  if (index === question.answer) {
    runnerQuizState.score += 1;
    user.coins += 10;
    user.gameStats.quizScore += 1;
    saveUser(user);
    runnerQuizState.feedback = "Correct. " + question.explain + " You earned 10 bonus coins.";
    playCorrectAnswerSound();
    celebrateReward({ coins: 10 });
  } else {
    runnerQuizState.feedback = "Good try. " + question.explain;
    playWrongAnswerSound();
  }
  if (runnerQuizState.index < runnerQuizState.questions.length - 1) {
    runnerQuizState.index += 1;
  } else {
    runnerQuizState.complete = true;
    runnerQuizState.feedback = "Great mission. Your runner quiz is saved on the dashboard.";
    playQuizScoreSound();
  }
  render();
}

function buyRunnerItem(itemId) {
  const user = getUser();
  const item = runnerShopItems.find((shopItem) => shopItem.id === itemId);
  if (!user || !item || user.runnerInventory.includes(item.id) || user.coins < item.cost) return;
  user.coins -= item.cost;
  user.runnerInventory.push(item.id);
  user.rewards.push(item.title);
  addRunnerBadge(user, "Style Shopper");
  saveUser(user);
  render();
  celebrateBadge(item.title);
}

function equipRunnerItem(itemId) {
  const user = getUser();
  const item = runnerShopItems.find((shopItem) => shopItem.id === itemId);
  if (!user || !item || !user.runnerInventory.includes(item.id)) return;
  user.runnerLoadout[item.type] = item.title;
  saveUser(user);
  render();
}
...
function moveRunner(direction) {
  if (!runnerState?.running) return;
  runnerState.lane = clamp(runnerState.lane + direction, 0, 2);
  drawRunnerGame();
}

function jumpRunner() {
  if (!runnerState?.running) return;
  runnerState.jumpTicks = 10;
  runnerState.slideTicks = 0;
  drawRunnerGame();
}

function slideRunner() {
  if (!runnerState?.running) return;
  runnerState.slideTicks = 10;
  runnerState.jumpTicks = 0;
  drawRunnerGame();
}
...
function drawRunnerGame() {
  ...
  gradient.addColorStop(0.46, "#8e7dff");
  gradient.addColorStop(1, "#172033");
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, width, height);
  drawRunnerCity(ctx, width, height);
  const roadTop = 110;
  const roadBottom = height;
  ctx.fillStyle = "#243451";
  ctx.beginPath();
  ctx.moveTo(width * 0.34, roadTop);
  ctx.lineTo(width * 0.66, roadTop);
  ctx.lineTo(width * 0.92, roadBottom);
  ctx.lineTo(width * 0.08, roadBottom);
  ctx.closePath();
  ctx.fill();
  [0.38, 0.5, 0.62].forEach((ratio) => {
    ctx.strokeStyle = "rgba(255,255,255,0.38)";
    ctx.lineWidth = 5;
    ctx.setLineDash([18, 18]);
    ctx.beginPath();
    ctx.moveTo(width * ratio, roadTop + 8);
    ctx.lineTo(width * (ratio + (ratio - 0.5) * 0.6), roadBottom);
    ctx.stroke();
  });
  ctx.setLineDash([]);
  const state = runnerState || { lane: 1, items: [], score: 0, coins: 0, lives: 3 };
  state.items?.forEach((item) => drawRunnerItem(ctx, item, width));
  drawRunnerAvatar(ctx, state, width, height);
  ctx.fillStyle = "rgba(255,255,255,0.92)";
  ctx.font = "bold 24px Inter, sans-serif";
  ctx.fillText("Score " + formatNumber(state.score || 0), 24, 36);
  ctx.fillText("Coins " + formatNumber(state.coins || 0), 24, 68);
  ctx.fillText("Lives " + formatNumber(state.lives || 3), width - 130, 36);
  if (!runnerState) {
    ctx.fillStyle = "rgba(7,17,31,0.76)";
    ctx.fillRect(width / 2 - 210, height / 2 - 48, 420, 96);
    ctx.fillStyle = "#ffffff";
    ctx.textAlign = "center";
    ctx.font = "900 28px Inter, sans-serif";
    ctx.fillText("Press Start Runner Mission", width / 2, height / 2 - 5);
    ctx.font = "700 16px Inter, sans-serif";
    ctx.fillText("Collect rights cards, coins, and reach the safe zone.", width / 2, height / 2 + 26);
    ctx.textAlign = "left";
  }
}

function drawRunnerCity(ctx, width, height) {
  ctx.fillStyle = "rgba(255,255,255,0.92)";
  ctx.beginPath();
  ctx.arc(width - 80, 72, 28, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(255,209,102,0.96)";
  ctx.beginPath();
  ctx.arc(94, 80, 34, 0, Math.PI * 2);
  ctx.fill();
  for (let index = 0; index < 9; index += 1) {
    const x = 70 + index * 82;
    const buildingHeight = 50 + (index % 4) * 20;
    ctx.fillStyle = index % 2 ? "rgba(184,244,210,0.72)" : "rgba(255,255,255,0.72)";
    ctx.fillRect(x, 105 - buildingHeight, 46, buildingHeight);
  }
}

function laneX(lane, width) {
  return [width * 0.36, width * 0.5, width * 0.64][lane] || width * 0.5;
}

function drawRunnerItem(ctx, item, width) {
  const x = laneX(item.lane, width);
  const y = item.y;
  if (item.type === "coin") {
    ...
    ctx.fillText("C", x - 6, y + 6);
  } else if (item.type === "card") {
    ctx.fillStyle = "#ffffff";
    ctx.strokeStyle = "#27ae7e";
    ctx.lineWidth = 4;
    ctx.roundRect?.(x - 36, y - 26, 72, 52, 12);
    if (!ctx.roundRect) ctx.rect(x - 36, y - 26, 72, 52);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = "#102036";
    ctx.font = "bold 14px Inter, sans-serif";
    ctx.fillText("RIGHT", x - 24, y + 5);
  } else {
    ctx.fillStyle = item.kind === "low" ? "#ff8a4c" : "#ff6f91";
    ctx.fillRect(x - 38, y - 20, 76, item.kind === "low" ? 24 : 62);
    ctx.fillStyle = "#ffffff";
    ctx.font = "bold 14px Inter, sans-serif";
    ctx.fillText(item.kind === "low" ? "SLIDE" : "JUMP", x - 24, y + 6);
  }
}

function drawRunnerAvatar(ctx, state, width, height) {
  const x = laneX(state.lane || 1, width);
  const jumpOffset = state.jumpTicks > 0 ? 72 : 0;
  const y = height - 76 - jumpOffset;
  const isSliding = state.slideTicks > 0;
  ctx.fillStyle = "#ffd166";
  ctx.beginPath();
  ctx.arc(x, y - 56, 25, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#3478f6";
  ctx.fillRect(x - 28, y - 36, 56, isSliding ? 28 : 68);
  ctx.fillStyle = "#ffffff";
  ctx.beginPath();
  ctx.arc(x - 9, y - 59, 4, 0, Math.PI * 2);
  ctx.arc(x + 9, y - 59, 4, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#ff6f91";
  ctx.fillRect(x - 34, y + (isSliding ? -4 : 30), 68, 10);
  ctx.fillStyle = "#b8f4d2";
  ctx.fillRect(x - 22, y - 82, 44, 14);
}
...
function jumpTo(id) {
  ...
  if (user && (id === "about" || id === "modules" || id === "videos" || id === "runner-game" || id === "quiz-zone" || id === "quiz-guide")) {
    view = "dashboard";
  } else {
    view = "home";
  }
...
document.addEventListener("keydown", (event) => {
  if (!runnerState?.running || event.target.closest("input, textarea, select")) return;
...
function handleAction(action, source = null) {
  ...
  if (action === "runner-left") {
    moveRunner(-1);
    return;
  }
  if (action === "runner-right") {
    moveRunner(1);
    return;
  }
  if (action === "runner-jump") {
    jumpRunner();
    return;
  }
  if (action === "runner-slide") {
    slideRunner();
    return;
  }
  if (action === "runner-buy") {
    buyRunnerItem(source?.dataset.runnerItem);
    return;
  }
  if (action === "runner-equip") {
    equipRunnerItem(source?.dataset.runnerItem);
    return;
  }
  if (action === "runner-quiz-answer") {
    answerRunnerQuiz(Number(source?.dataset.runnerAnswer));
    return;
  }
}
